...
namespace Es.InkPainter
{
	...
	public class InkCanvas : MonoBehaviour
	{
		...
		[SerializeField]
		protected List<PaintSet> paintSet = null;
		...
		/// <param name="renderCamera">Camera to use to render the object.</param>
		/// <returns>The success or failure of the paint.</returns>
		public virtual bool Paint(Brush brush, Vector3 worldPos, Camera renderCamera = null)
		{
			Vector2 uv;
		...
		/// To reset the paint.
		/// </summary>
		public virtual void ResetPaint()
		{
			ReleaseRenderTexture();
			SetRenderTexture();
			if(OnInitializedAfter != null)
